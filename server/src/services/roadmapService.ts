...
export const roadmapService = {
  ...
  /**
   * Story G — avança o desdobramento até o "pedido de proposta": liga/cria um Deal
   * na etapa PROPOSAL (reusa um Deal aberto da empresa/empreendimento, senão cria) e
   * move o roadmap para PROPOSTA.
   */
  async advanceToProposal(tenantId: string, roadmapId: string, ownerId?: string) {
    const roadmap = await prisma.commercialRoadmap.findFirst({
      where: { id: roadmapId, tenantId, deletedAt: null },
    });
    if (!roadmap) throw createError('Desdobramento não encontrado', 404, 'ROADMAP_NOT_FOUND');

    let dealId = roadmap.dealId;
    if (dealId) {
      await prisma.deal.update({ where: { id: dealId }, data: { stage: DealStage.PROPOSAL } });
    } else {
      const open = await prisma.deal.findFirst({
        where: {
          tenantId,
          companyId: roadmap.companyId,
          empreendimentoId: roadmap.empreendimentoId,
    ...
    await prisma.commercialRoadmap.update({
      where: { id: roadmapId },
      data: { dealId, status: CommercialRoadmapStatus.PROPOSTA },
    });
    return this.findById(tenantId, roadmapId);
  },

  /**
   * Story H — painel "não deixar passar": próximas ações, atrasadas e roadmaps em
   * risco (ativos com ação de acesso já vencida). Segmentável por vendedor.
   */
  async getPanel(tenantId: string, opts?: { assignedTo?: string; riskDays?: number }) {
    const now = new Date();
    const riskDays = opts?.riskDays ?? 7;
    const horizon = new Date(now);
    horizon.setDate(horizon.getDate() + riskDays);
    ...
    const [upcoming, overdue, atRisk] = await Promise.all([
      prisma.task.findMany({
        where: { ...base, dueDate: { gte: now, lte: horizon } },
        orderBy: { dueDate: 'asc' },
        take: 50,
        include: taskInclude,
      }),
      ...
      prisma.commercialRoadmap.findMany({
        where: {
          ...
          status: {
            notIn: [
              CommercialRoadmapStatus.GANHO,
              CommercialRoadmapStatus.PERDIDO,
              CommercialRoadmapStatus.ARQUIVADO,
            ],
          },
          tasks: {
            some: {
