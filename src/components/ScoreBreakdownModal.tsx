        ...
        <DialogHeader className="text-left space-y-0 pb-4">
          ...
          {data && (
            <p className="text-sm text-gray-500 mt-1">
              {data.leadName} &mdash;{" "}
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${getScoreColor(data.totalScore)}`}>
                {data.totalScore} pts
              </span>
            </p>
          )}
          ...
          <div className="space-y-4">
            {data.breakdown.length > 0 ? (
                ...
                <table className="w-full text-sm" aria-label="Breakdown do score">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left py-2 pr-3 text-xs font-medium text-gray-500 uppercase">Regra</th>
                      <th className="text-left py-2 pr-3 text-xs font-medium text-gray-500 uppercase">Evento</th>
                      <th className="text-right py-2 pr-3 text-xs font-medium text-gray-500 uppercase">Pts/Evento</th>
                      <th className="text-right py-2 pr-3 text-xs font-medium text-gray-500 uppercase">Qtd</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {data.breakdown.map((row) => (
                      <tr key={row.ruleId} className="hover:bg-gray-50">
                        ...
                        <td className="py-2 pr-3 text-gray-600">{EVENT_LABELS[row.event] || row.event}</td>
                        ...
                        <td className="py-2 text-right font-semibold text-gray-900">{row.totalPoints}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="border-t-2 border-gray-300">
                      ...
                      <td className="py-2 text-right">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${getScoreColor(data.calculatedScore)}`}>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 text-xs mt-1">
                  Configure regras de scoring em Configuracoes para calcular o score automaticamente.
                </p>
              </div>
            )}
            ...
            {data.totalScore !== data.calculatedScore && data.breakdown.length > 0 && (
              <p className="text-xs text-gray-400 italic">
                O score armazenado ({data.totalScore}) pode diferir do calculado ({data.calculatedScore}) se regras foram alteradas desde o ultimo calculo.
              </p>
            )}
          </div>
        ) : null}
