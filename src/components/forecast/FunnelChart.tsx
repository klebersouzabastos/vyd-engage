...
export function FunnelChart({ stages, total, compact = false }: FunnelChartProps) {
  ...
  const funnelStages = useMemo(() => stages.filter((s) => s.stage !== 'LOST'), [stages]);

  const lostStage = useMemo(() => stages.find((s) => s.stage === 'LOST'), [stages]);
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 relative">
                  ...
                  >
                    ...
                    <span className="text-white/80 text-xs ml-2">({item.pctOfTotal}%)</span>
                  </div>
                </div>
                {/* Conversion arrow */}
                {item.conversionToNext !== null && idx < chartData.length - 1 && (
                  <div className="w-20 md:w-24 text-left flex flex-col items-start">
                    ...
                    {item.dropOffRate !== null && item.dropOffRate > 0 && (
                      <span className="text-xs text-red-500">-{item.dropOffRate}%</span>
                    )}
                  </div>
                )}
              </div>
          ...
          >
            ...
            <Tooltip
              formatter={(value: number, _name: string, props: any) => {
                ...
                return [`${value} leads (${item.pctOfTotal}%)`, item.name];
              }}
...
function CompactFunnel({ stages, total }: { stages: FunnelConversionStage[]; total: number }) {
        ...
        return (
          <div key={s.stage} className="flex items-center gap-2">
            <div className="w-16 text-right">
              <span className="text-xs text-gray-600">{STAGE_NAMES[s.stage] || s.stage}</span>
            </div>
            <div className="flex-1 relative">
              <div
            ...
            {s.conversionToNext !== null && idx < topStages.length - 1 && (
              <span className="text-xs text-green-600 w-10 text-right">{s.conversionToNext}%</span>
            )}
          </div>
        );
      })}
