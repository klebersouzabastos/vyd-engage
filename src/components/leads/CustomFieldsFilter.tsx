...
export function CustomFieldsFilter({
  ...
  const activeCount = Object.keys(filterCustomFields).filter(
    (k) =>
      filterCustomFields[k] !== null &&
      filterCustomFields[k] !== undefined &&
      filterCustomFields[k] !== ''
  ).length;
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <Filter size={16} />
        <span>{activeCount === 0 ? 'Campos Customizados' : `${activeCount} campo(s)`}</span>
        <ChevronDown
          size={16}
          className={
            open
