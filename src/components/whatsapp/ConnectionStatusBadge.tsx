    ...
    <div className="flex items-center gap-2">
      ...
      {showBattery && BatteryIcon && status.batteryLevel !== undefined && (
        <div className="flex items-center gap-1 text-xs text-gray-600">
          {/* eslint-disable-next-line react-hooks/static-components -- BatteryIcon é referência a um ícone lucide-react existente selecionado por valor, não um componente criado em render */}
      ...
      {showLastSync && status.lastSync && (
        <span className="text-xs text-gray-600">{formatLastSync(status.lastSync)}</span>
      )}
