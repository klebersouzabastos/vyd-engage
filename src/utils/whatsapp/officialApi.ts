 ...
 */
export async function getOfficialStatus(config: OfficialConfig): Promise<ConnectionStatusInfo> {
  try {
    const response = await fetch(`${WHATSAPP_API_BASE}/${config.phoneNumberId}`, {
      method: 'GET',
      headers: {
 ...
 */
export async function getOfficialTemplates(config: OfficialConfig): Promise<WhatsAppTemplate[]> {
  try {
    const businessAccountId = config.businessAccountId || config.phoneNumberId;
    const response = await fetch(`${WHATSAPP_API_BASE}/${businessAccountId}/message_templates`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${config.accessToken}`,
      },
