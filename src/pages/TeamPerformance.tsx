...
async function fetchTeamPerformance(period: Period): Promise<TeamPerformanceResponse> {
  ...
  const res = await fetch(
    `${API_BASE}/api/v1/reports/team-performance?from=${from}&to=${to}`,
    {
      headers: {
...
export function TeamPerformance() {
  ...
  const isAllowed = user?.role === "ADMIN" || user?.role === "GESTOR" || user?.role === "admin" || user?.role === "gestor";
  ...
  return (
    <div className="min-h-screen">
      <Header
        ...
        subtitle="Análise de desempenho por vendedor"
      />
                ...
                <tbody>
                  {sortedMembers.map((member) => (
                    <tr key={member.userId} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-sm text-gray-700 text-right">{member.dealsWon}</td>
                      <td className="py-3 px-4 text-sm font-medium text-green-600 text-right">
                        {formatCurrency(member.revenueWon)}
                      </td>
                      <td className="py-3 px-4 text-sm text-red-500 text-right">{member.dealsLost}</td>
                      ...
                      <td className="py-3 px-4 text-sm text-gray-600 text-right">{member.avgCycleDays}</td>
                      <td className="py-3 px-4 text-sm text-blue-600 text-right">
                        {formatCurrency(member.pipelineValue)}
                      </td>
                      <td className="py-3 px-4 text-sm text-gray-600 text-right">{member.tasksDone}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
