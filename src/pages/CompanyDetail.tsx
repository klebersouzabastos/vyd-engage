...
export function CompanyDetail() {
      ...
      <div className="min-h-screen">
        ...
        <div className="p-8 text-center">
          ...
          <Button onClick={() => navigate("/app/companies")}>
            <ArrowLeft size={16} className="mr-2" />Voltar para Empresas
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-8">
        <button onClick={() => navigate("/app/companies")} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6 transition-colors">
          <ArrowLeft size={16} /><span className="text-sm">Voltar para Empresas</span>
        </button>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-300 p-6 mb-6">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 mt-1 text-sm text-gray-500">
                  ...
                  {company.size && <span className="px-2 py-0.5 bg-gray-100 rounded text-xs">{SIZE_LABELS[company.size]}</span>}
                  {company.domain && (
            ...
            <Button variant="outline" size="sm" className="gap-2" onClick={() => setEditFormOpen(true)}>
          ...
          <div className="flex gap-0">
            {tabs.map(tab => (
              ...
              >
                {tab.label}
                {tab.count !== null && (
                  <span className="ml-2 px-1.5 py-0.5 text-xs bg-gray-100 rounded-full">{tab.count}</span>
                )}
              </button>
            ))}
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="text-left py-3 px-4 text-xs font-medium text-gray-500 uppercase">Criado</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="text-left py-3 px-4 text-xs font-medium text-gray-500 uppercase">Probabilidade</th>
                    <th className="text-left py-3 px-4 text-xs font-medium text-gray-500 uppercase">Criado</th>
                  </tr>
                </thead>
                <tbody>
                  {company.deals.map(deal => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STAGE_COLORS[deal.stage] || "bg-gray-100 text-gray-700"}`}>
                          {deal.stage}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-sm text-gray-600">{deal.probability}%</td>
                      <td className="py-3 px-4 text-sm text-gray-500">{formatDate(deal.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                    ...
                    <div key={interaction.id} className="relative flex gap-4 py-3 pl-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-gray-400 ml-auto">{formatRelativeTime(interaction.createdAt)}</span>
                        </div>
        ...
        {activeTab === "info" && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-300 p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <InfoItem icon={<Building2 size={16} />} label="Porte" value={company.size ? SIZE_LABELS[company.size] : null} />
              <InfoItem icon={<Phone size={16} />} label="Telefone" value={company.phone} />
              <InfoItem icon={<LinkIcon size={16} />} label="Website" value={company.website} isLink />
      ...
      {/* Edit form dialog */}
      <Dialog open={editFormOpen} onOpenChange={(open) => { if (!open) setEditFormOpen(false); }}>
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        ...
        {value ? (
          isLink ? (
            <a href={value.startsWith("http") ? value : `https://${value}`} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">
              {value}
            </a>
          ) : (
